  ...
  return (
    <div className="pl-[4vw] w-full flex items-center">
      <button onClick={() => router.push('/home')} className="active:scale-[0.95] transition-all mr-[3.4vw]">
        <svg className="w-34px h-34px" width="34" height="34" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {subcategoryList.map((item, index) => (
          <SwiperSlide key={index} className='max-w-fit'>
            <div 
              ...
              className={`h-40px w-fit transition-all active:scale-95 flex items-center justify-center p-[0px_6vw] fs-13 rounded-[16px] font-medium ${activeIndex === index ? 'text-[#23262F] bg-[#F7F7F8]': 'text-[#AAAAAB] bg-[#F7F7F81A]'}`}
            >
              {item}
            </div>
          </SwiperSlide>
