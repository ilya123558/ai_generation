...
export const ChatPrompt = () => {
  ...
  return (
    <div className={`w-full h-full bg-white mt-[2.67vw] rounded-[30px_30px_0px_0px] p-[3.8vw_4.5vw]`}>
      <div className="flex items-center gap-[10px] mb-[2.8vw]">
        <button onClick={() => setToggle(!toggle)} className={`relative`}>
          <div className={`w-45px h-26px transition-all rounded-[15px] overflow-hidden border-[2px] ${toggle ? 'bg-[#23262F] border-transparent': 'bg-["#BEBEBE"] border-[#BEBEBE]'}`}></div>
        ...
        <div className="relative w-fit">
          ...
          <button className="active:scale-95 transition-all absolute right-[-4.5vw] top-[-1vw]">
      ...
      <div className={`${isFocus ? 'fixed w-full h-full left-0 top-0 z-[20] flex items-end backdrop-blur-[20px]': ''} transition-all`}>
        <div className={`${isFocus ? 'mb-[70vh] w-full p-[0px_4vw]': ''} transition-all relative`}>
          <textarea 
            value={prompt} 
      ...
      <EllipseButton onClick={() => {}} className="mt-[2.8vw]">Generate</EllipseButton>
    </div>
  );
};