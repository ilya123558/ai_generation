  ...
  <>
    {
      images.map((item, index) => (
          ...
          >
            ...
            />
            <button onClick={() => handleDelete(item)} className='h-28px w-28px bg-primary rounded-full flex items-center justify-center absolute top-[6px] right-[6px] z-[10]'>
      ...
      {
        Array(maxImages - images.length).fill(null).map((_, index) => (
          <div
            key={index}
            style={{
              width: size ? `${size.maxWidth * 0.267}vw` : `${100 * 0.267}vw`,
